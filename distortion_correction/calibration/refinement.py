...
def pack_params(K, k, extrinsic_matrices):
    ...
    for E in extrinsic_matrices:
        ...
        rho_x, rho_y, rho_z = rodrigues[0, 0], rodrigues[1, 0], rodrigues[2, 0]
        t_x, t_y, t_z = t
...
def unpack_refinement_params(params):
    ...
    # Unpack intrinsics
    alpha, u_c, v_c, k0, k1 = intrinsics
    K = np.array([[alpha, 0,  u_c],
                  [   0.,  alpha, v_c],
                  [   0.,    0.,  1.]])
    k = np.array([k0, k1])
...
def f_refine_all(xdata, *params):
    ...
    if len(params) < 5 or len(params[5:]) % 6 != 0:
